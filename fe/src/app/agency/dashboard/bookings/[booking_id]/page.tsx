"use client";

import React, { useEffect, useMemo, useState } from "react";
import { useParams } from "next/navigation";
import { toast } from "sonner";

import { useBookingService } from "@/services/bookingService";
import type { BookingDetail } from "@/types/booking";
import AgencyBookingDetailView from "@/components/agency/booking/AgencyBookingDetailView";
import BookingDetailSkeleton from "./BookingDetailSkeleton";

const AgencyBookingDetailPage = () => {
  const params = useParams();

  const bookingId = useMemo(
    () => (params?.booking_id as string) || "",
    [params]
  );

  const { getDetailBookingAgency } = useBookingService();

  const [booking, setBooking] = useState<BookingDetail | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let mounted = true;

    const fetchData = async () => {
      if (!bookingId) return;

      setLoading(true);
      try {
        const res = await getDetailBookingAgency(bookingId);

        if (!mounted) return;

        if (!res?.success) {
          toast.error(
            typeof res?.message === "string"
              ? res.message
              : "Không lấy được chi tiết booking."
          );
          setBooking(null);
          return;
        }

        setBooking((res.data ?? null) as BookingDetail | null);
      } catch (error) {
        console.error(error);
        toast.error("Lỗi hệ thống, vui lòng thử lại sau.");
        setBooking(null);
      } finally {
        if (mounted) setLoading(false);
      }
    };

    fetchData();
    return () => {
      mounted = false;
    };
  }, [bookingId]);

  if (loading)
    return (
      <div className="p-6">
        <BookingDetailSkeleton />
      </div>
    );
  if (!booking) return <div className="p-6">Không tìm thấy booking.</div>;

  return <AgencyBookingDetailView booking={booking} />;
};
export default AgencyBookingDetailPage;
