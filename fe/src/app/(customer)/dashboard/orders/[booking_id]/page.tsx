...
const CustomerBookingDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getDetailBookingCustomer(booking_id as string);
  ...
  const handelSubmitReview = async (payload: ReviewPayload) => {
    try {
      const res = await createReview(payload);
      console.log("Api response khi tạo review: ", res);
      if (res.success) {
        toast.success(
