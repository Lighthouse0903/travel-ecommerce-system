...
class TourListCreateView(generics.ListCreateAPIView):
    ...
    search_fields = ['name', 'start_location', 'end_location', 'agency__company_name', 'price', 'region', 'category']
    ordering_fields = ['price', 'created_at', 'duration_days']

    def perform_create(self, serializer):
        # gán agency theo user đăng nhập
...
class PublicTourListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # ---- 1) Filter bằng DB (tận dụng index) ----
        # agency name
        ...
        # region: chấp nhận "1,2" hoặc "north,central"
        region_param = self.request.query_params.get('region')
        if region_param:
            alias = {
                'north': Tour.NORTH,
                'central': Tour.CENTRAL,
                'south': Tour.SOUTH,
            }
        ...
        # categories: chấp nhận "sea,mountain" (ANY match)
        # - overlap: chỉ cần trùng ANY phần tử
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not queryset:
            return Response({"message": "Không tìm thấy kết quả."}, status=status.HTTP_404_NOT_FOUND)
